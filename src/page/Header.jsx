  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-14' : 'h-16 sm:h-20'}`}>
          ...
          <a href="#" className="flex items-center gap-3 group shrink-0">
            ...
            <div className={`hidden lg:flex flex-col border-l pl-3 ml-1 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          ...
          <nav className="hidden md:flex items-center gap-1 lg:gap-1.5">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className={`relative px-3 py-2 text-sm font-semibold rounded-xl transition-all duration-200 group ${
                  isDark
              ...
              >
                {link.name}
                <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-pink-500 rounded-full group-hover:w-4/5 transition-all duration-300" />
              </a>
            ))}

            <button
              onClick={onOpenOrders}
              className={`relative px-3 py-2 text-sm font-semibold rounded-xl transition-all duration-200 flex items-center gap-1.5 group cursor-pointer ${
                isDark
                  ? 'text-slate-300 hover:text-pink-400 hover:bg-slate-800/60'
                  : 'text-slate-600 hover:text-pink-600 hover:bg-pink-50/80'
              }`}
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <button
              onClick={onOpenCart}
              className={`relative px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-2xl font-bold text-xs sm:text-sm flex items-center gap-2 border transition-all active:scale-95 cursor-pointer ${
                isDark
            ...
            {currentUser ? (
              <div
                className={`flex items-center gap-2 p-1.5 sm:pr-2.5 rounded-2xl border transition-all ${
                  isDark
              ...
              >
                <img
                  ...
                  className="w-8 h-8 rounded-xl object-cover border border-pink-300 shrink-0"
                />
                <div className="hidden sm:flex flex-col text-left">
                  <span className={`text-xs font-bold leading-tight truncate max-w-[110px] ${isDark ? 'text-white' : 'text-slate-900'}`}>
                    {currentUser.name}
                  </span>
                  <span className="text-[10px] font-bold text-pink-600 leading-none">
                ...
                <button
                  onClick={onLogout}
                  className="flex items-center gap-1 p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all cursor-pointer font-bold text-xs"
                  title="Hisobdan chiqish / Boshqa hisob tanlash"
                >
                  <span className="text-sm">🚪</span>
                  <span className="hidden xl:inline text-[11px] text-rose-500">Chiqish</span>
                </button>
              </div>
            ) : (
              <button
                onClick={onOpenAuthModal}
                className="btn-pink px-4 sm:px-5 py-2.5 rounded-2xl text-xs sm:text-sm font-bold flex items-center gap-1.5 shadow-md shadow-pink-500/25 hover:shadow-pink-500/40 cursor-pointer"
              >
                <span>👤</span>
            ...
            <button
              ...
              className={`p-2.5 rounded-xl transition-all duration-200 md:hidden cursor-pointer ${
                mobileMenuOpen
